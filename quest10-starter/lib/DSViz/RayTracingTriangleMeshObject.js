...
export default class RayTracingTriangleMeshObject extends RayTracingObject {
  constructor(device, canvasFormat, filename, camera) {
    ...
    this._camera = camera;
    this._aabb = null; // Initialize AABB to null
  }
  ...
  async createGeometry() {
    ...
    // Create AABB buffer to store the AABB min and max points
    this._aabbBuffer = this._device.createBuffer({
      label: "AABB " + this.getName(),
      size: 6 * Float32Array.BYTES_PER_ELEMENT, // 6 floats: minX, minY, minZ, maxX, maxY, maxZ
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    
    // Copy AABB data to GPU
    const aabbData = new Float32Array([
      this._aabb.min[0], this._aabb.min[1], this._aabb.min[2],
      this._aabb.max[0], this._aabb.max[1], this._aabb.max[2]
    ]);
    this._device.queue.writeBuffer(this._aabbBuffer, 0, aabbData);
  }
  ...
  _computeAABB() {
    ...
    // Iterate through all vertices to find min and max values for each axis
    for (let i = 0; i < this._vertices.length; i += this._vProp.length) {
  ...
  createBindGroup(outTexture) {
    ...
    this._bindGroup = this._device.createBindGroup({
      ...
      {
        binding: 4,
        resource: { buffer: this._aabbBuffer }
      }
      ],
    });
