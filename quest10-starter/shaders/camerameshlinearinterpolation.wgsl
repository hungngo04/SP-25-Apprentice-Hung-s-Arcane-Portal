...
fn vertexMain(
  @location(0) pos0: vec3f, @location(1) normal0: vec3f, 
  @location(2) pos1: vec3f, @location(3) normal1: vec3f,
  @location(4) pos2: vec3f, @location(5) normal2: vec3f
) -> VertexOut {
  // Get the current and next keyframe indices
  let current = i32(cameraPose.current_keyframe);
  let next = i32(cameraPose.next_keyframe);
  
  // Get positions and normals for current and next keyframes
  var currentPos = vec3f(0.0, 0.0, 0.0);
  var currentNormal = vec3f(0.0, 0.0, 0.0);
  var nextPos = vec3f(0.0, 0.0, 0.0);
  var nextNormal = vec3f(0.0, 0.0, 0.0);
  
  // Select the appropriate keyframes based on indices
  if (current == 0) {
    currentPos = pos0;
    currentNormal = normal0;
  } else if (current == 1) {
    currentPos = pos1;
    currentNormal = normal1;
  } else if (current == 2) {
    currentPos = pos2;
    currentNormal = normal2;
  }
  ...
  // Interpolate between current and next keyframes
  var interpolatePt = currentPos * (1.0 - cameraPose.t) + nextPos * cameraPose.t;
  var interpolateNormal = currentNormal * (1.0 - cameraPose.t) + nextNormal * cameraPose.t;
...
fn fragmentMain(@location(0) normal: vec3f) -> @location(0) vec4f {
  ...
  let lightDir = normalize(vec3f(1.0, 1.0, 1.0)); // Light direction
  let lightColor = vec3f(1.0, 1.0, 1.0); // White light
  let ambientStrength = 0.2; // Ambient light intensity
  let diffuseStrength = 0.6; // Diffuse light intensity
  let specularStrength = 0.4; // Specular light intensity
  let shininess = 32.0; // Shininess factor
  
  // Define material properties
  let materialColor = vec3f(0.7, 0.2, 0.3); // Base material color (reddish)
  ...
  // Calculate specular component (view direction is assumed to be the same as light direction for simplicity)
  ...
  let result = (ambient + diffuse + specular) * materialColor;
