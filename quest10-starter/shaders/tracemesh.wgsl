...
// Returns the t value of intersection, or -1 if no intersection
fn rayAABBIntersection(s: vec3f, d: vec3f, aabb: AABB) -> f32 {
  ...
  // For each axis (x, y, z)
  for (var i = 0; i < 3; i++) {
    // Check if ray direction is parallel to the slab
    if (abs(d[i]) < EPSILON) {
      // Ray is parallel to slab, check if ray origin is outside slab
      if (s[i] < aabb.min[i] || s[i] > aabb.max[i]) {
        return -1.0; // No intersection
      }
    } else {
      // Compute intersection t values with near and far slab planes
      var t1 = (aabb.min[i] - s[i]) / d[i];
      var t2 = (aabb.max[i] - s[i]) / d[i];
  ...
  // Return the t value of intersection
...
@group(0) @binding(4) var<uniform> aabb: AABB;                       // AABB for the mesh
...
fn getColor(p: vec3f, d: vec3f) -> vec4f {
  ...
  // First check if the ray hits the AABB
  let aabbHit = rayAABBIntersection(p, d, aabb);
  if (aabbHit < 0.0) {
    // Ray doesn't hit the AABB, return background color
    return color;
  }
  
  // Ray hits the AABB, now check individual triangles
  var t = -1.;
  var minIdx = -1;
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    // compute the pixel size
    let psize = vec2f(2, 2) / cameraPose.res.xy;
    // compute the ray direction for perspective camera
    var spt = vec3f(0, 0, 0); // camera origin
    var rdir = vec3f((f32(uv.x) + 0.5) * psize.x - 1, (f32(uv.y) + 0.5) * psize.y - 1, 1);
    rdir = normalize(rdir * vec3f(1.0 / cameraPose.focal.x, 1.0 / cameraPose.focal.y, 1.0));
